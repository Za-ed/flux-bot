...
async function handleChillMessage(message) {
    ...
    try {
        ...
        const dialectMap = {
            saudi:    'السعودية (ابشر، الحين، يا رجال)',
            jordanian:'الأردنية (هيك، هلأ، زلمة، شو)',
            egyptian: 'المصرية (إيه، كده، دلوقتي)',
            gulf:     'الخليجية (وايد، صج، تره)',
            levantine:'الشامية (كيفك، منيح، يلا)',
            english:  'English casual only',
        };
        const dl = dialectMap[perception.dialect] || 'العربية العامية السهلة';
        const sysPrompt = `أنت FLUX، عضو حقيقي في سيرفر FLUX IO.
${evolutionDesc}
قواعد صارمة:
- تحدث بالعربية العامية فقط — لهجة ${dl}
- ممنوع أي كلمة أجنبية أو روسية
- ممنوع لغة رسمية أو عبارات مثل "سأحاول التواصل" أو "يمكنني"
