...
// ─── البناء الرئيسي للـ GIF ────────────────────────────────────────────────
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    ctx.font = 'bold 42px Bricolage, Arial, sans-serif';
    ctx.fillStyle = '#ffffff';
    ctx.shadowColor = tier.glow;
    ctx.shadowBlur = 12 * pulse;
    ctx.fillText(displayName || username || 'Unknown', TX, 75); // تم تعديل الارتفاع قليلاً
    ctx.restore();

    // 2. الرتبة والمستوى
    const tierLabel = `${tier.label}  LVL.${level}`;
    ctx.font = 'bold 14px GeistMono, Arial';
    const labelW = ctx.measureText(tierLabel).width + 30;
    ctx.fillStyle = `rgba(${gr},${gg},${gb},0.25)`;
    roundRect(ctx, TX, 105, labelW, 28, 14); // رفعنا الصندوق قليلاً للأعلى ليتناسق
    ctx.fill();
    ctx.fillStyle = tier.glow;
    ctx.fillText(tierLabel, TX + 15, 120);
    ...
    ctx.textAlign = 'right';
    ctx.fillText(xpText, TX + 450, 155); // محاذاة دقيقة مع نهاية الشريط

    // 4. رسم شريط التقدم
    const BX = TX, BY = 175, BW = 450, BH = 14;
